...
async def submit_passport_application(
    application: PassportApplication, db: Session = Depends(get_db)
):
...
async def get_application_status(application_id: str, db: Session = Depends(get_db)):
    ...
    application = (
        db.query(PassportApplicationDB)
        .filter(PassportApplicationDB.application_id == application_id)
        .first()
    )
...
async def get_applications(
    ...
    applications = (
        db.query(PassportApplicationDB)
        .order_by(PassportApplicationDB.submission_date.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
